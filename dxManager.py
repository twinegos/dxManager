...
class DxManager(QMainWindow):
    def __init__(self,parent=None):
        ...
        self.listViewSlider.valueChanged.connect(self.ui_controller.setListviewLineEdit)

        self.listViewSlider.sliderReleased.connect(self.ui_controller.changeListViewUI_notTracking)

        # 초기 기본 5개의 리스트뷰 생성
        self.ui_controller.setUp_listViewUI(jsonData)
    ...
    def find_schedule(self, indexs):
        ...
        if task_date != []:
            ...
            if remainder == 1:
                self.ui_controller.changeListViewUI_(days_difference+4)
                schedule_EndDate = start + timedelta(days=days_difference+2)

            else:                
                self.ui_controller.changeListViewUI_(days_difference+2)
            ...
            for i in range(num_listview):
                ...
                if listview_layout:
                    ...
                    tasks = self.ui_controller.getListViewItem(listview) # 현재 ui상에 디스플레이되어있는 아이템들 가져오기
    ...
    def add_schedule(self, indexs):
        ...
        if scheduleRange != []:
            ...
            self.ui_controller.changeListViewUI_(total_addDays)
    ...
    def addListView(self, num, currentNum, scaleUI):
        ...
        return self.ui_controller.addListView(num, currentNum, scaleUI, activeJsonData)
    ...
    def updateJson(self,direction):
        ...
        for i in range(num_listview):
            ...
            if listview_layout:
                ...
                tasks = self.ui_controller.getListViewItem(listview) # 현재 ui상에 디스플레이되어있는 아이템들 가져오기
                tasks_inJson = self.ui_controller.getListViewItem_inJson(listview, checkedMembers)
        ...
        listView_date = self.ui_controller.getCurrentListViewDate(0)
    ...
    def move_sideways(self, direction, scheduleData):
        ...
        old_listViewDate = self.ui_controller.getCurrentListViewDate(0)
        listViewDate = self.ui_controller.getCurrentListViewDate(direction)
        ...
        for i in range(len(listViewDate)):
            ...
            self.ui_controller.set_listView_view(listViews[i], shotlist_model)
    ...
    def reloadShotList(self):
        ...
        if self.select_ctxMenu != 1 or self.reload_tatic == 1:  # 컨텍스트 메뉴 select All 이 아니거나 refresh 버튼이 눌려진 경우
            ...
            self.ui_controller.set_listView_view(self.shotListview, shotlist_model)
